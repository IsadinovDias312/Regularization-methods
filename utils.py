...
def get_network(args):
    ...
    if args.net == 'vgg16':
        ...
        net = resnet50()
    elif args.net == 'resnet50_bnless':
        from models.resnet import resnet50_bnless
        net = resnet50_bnless()
...
def get_training_dataloader(mean, std, batch_size=16, num_workers=2, shuffle=True, cutout=False, n_holes=1, length=8):
    ...
    if cutout:
        transform_train.transforms.append(Cutout(n_holes, length))
...
def get_training_dataloader_2(mean, std, batch_size=16, num_workers=2, shuffle=True):
    ...
    mnist_training = datasets.MNIST(root='./data', train=True, download=True, transform=transform_train)
...
def get_test_dataloader_2(mean, std, batch_size=16, num_workers=2, shuffle=True):
